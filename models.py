...
class ComunicadoPaciente(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    titulo = db.Column(db.String(150), nullable=False)
    mensagem = db.Column(db.Text, nullable=False)
    data_criacao = db.Column(db.DateTime, default=datetime.datetime.now, nullable=False)
    # Permite enviar para todos os pacientes (paciente_id nulo) ou para 1 específico
    paciente_id = db.Column(db.Integer, db.ForeignKey('paciente.id'), nullable=True)
    enviado_por = db.Column(db.String(100), nullable=False)  # Nome do admin

    def __repr__(self):
        return f"ComunicadoPaciente('{self.titulo}', Paciente: '{self.paciente_id}')"